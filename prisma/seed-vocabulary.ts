...
const TOPICS = [
  ...
  },
  {
    ...
    nameVi: "Con người & Quan hệ",
    description: "Words about people and relationships",
    icon: "👥",
    order: 3,
    keywords: [
      "people",
  ...
  },
  {
    ...
    nameVi: "Địa điểm & Vị trí",
    description: "Words about places and locations",
    icon: "📍",
    order: 5,
    keywords: [
      "place",
  ...
  },
  {
    ...
    nameVi: "Số & Số lượng",
    description: "Numbers and quantity words",
    icon: "🔢",
    order: 6,
    keywords: [
      "one",
  ...
  },
  {
    name: "Communication",
    nameVi: "Giao tiếp",
    description: "Words about communication",
    icon: "💬",
    order: 8,
    keywords: [
      "tell",
  ...
  {
    name: "Life & Society",
    nameVi: "Cuộc sống & Xã hội",
    description: "Social and life-related words",
    icon: "🌍",
    order: 9,
    keywords: [
      "life",
...
// Sample Vietnamese translations (simplified - you should use a proper API)
const SAMPLE_TRANSLATIONS: Record<string, any> = {
  the: {
  ...
  and: {
    meaning: "Và",
    example: "You and I",
    exampleVi: "Bạn và tôi",
    phonetic: "/ænd/",
  },
  have: {
    meaning: "Có, sở hữu",
    example: "I have a car",
    exampleVi: "Tôi có một chiếc xe",
    phonetic: "/hæv/",
  },
  that: {
    meaning: "Cái đó, rằng",
    example: "That is my book",
    exampleVi: "Đó là quyển sách của tôi",
    phonetic: "/ðæt/",
  },
  for: {
    meaning: "Cho, vì",
    example: "This is for you",
    exampleVi: "Cái này dành cho bạn",
    phonetic: "/fɔːr/",
  },
  // Add more as needed...
};

// Function to categorize word into topic
function categorizeWord(word: string): number {
  for (let i = 0; i < TOPICS.length; i++) {
    if (TOPICS[i].keywords.includes(word.toLowerCase())) {
      return i;
    }
  }
...
function determineLevel(index: number, total: number): string {
  const ratio = index / total;
  if (ratio < 0.5) return String(Level.BEGINNER);
  if (ratio < 0.8) return String(Level.INTERMEDIATE);
  return String(Level.ADVANCED);
}

// Generate translation (placeholder - replace with real API)
function generateTranslation(word: string) {
  if (SAMPLE_TRANSLATIONS[word]) {
    return SAMPLE_TRANSLATIONS[word];
  }

  return {
    meaning: `Nghĩa của ${word}`,
    example: `Example sentence with ${word}`,
    exampleVi: `Câu ví dụ với ${word}`,
    phonetic: `/${word}/`,
  };
}

// Generate exercises for a vocabulary
function generateExercises(vocab: any, allWords: string[]): any[] {
  const exercises = [];

  // 1. Multiple Choice - Translation
  const wrongOptions = allWords
    .filter((w) => w !== vocab.word)
  ...
  exercises.push({
    ...
    options: [vocab.meaningVi, ...wrongOptions.map((w) => `Nghĩa của ${w}`)],
    correctAnswer: vocab.meaningVi,
    explanation: `${vocab.word} means "${vocab.meaningVi}"`,
  });

  // 2. Fill in the blank
  const blankExample = vocab.example.replace(vocab.word, "______");
  exercises.push({
    type: String(ExerciseType.FILL_BLANK),
    question: `Fill in the blank: ${blankExample}`,
    questionVi: `Điền vào chỗ trống: ${vocab.exampleVi}`,
    options: [],
    correctAnswer: vocab.word,
    explanation: `The correct word is "${vocab.word}"`,
  });

  // 3. True/False
  exercises.push({
    type: String(ExerciseType.TRUE_FALSE),
    question: `"${vocab.word}" means "${vocab.meaningVi}"`,
    questionVi: `"${vocab.word}" có nghĩa là "${vocab.meaningVi}"`,
    options: ["True", "False"],
    correctAnswer: "True",
    explanation: `This is correct. ${vocab.word} = ${vocab.meaningVi}`,
  });

  // 4. Matching (phonetic)
  exercises.push({
    type: String(ExerciseType.MATCHING),
    question: `Match the word with its pronunciation: ${vocab.word}`,
    questionVi: `Ghép từ với cách phát âm: ${vocab.word}`,
    options: [vocab.phonetic || `/${vocab.word}/`],
    correctAnswer: vocab.phonetic || `/${vocab.word}/`,
    explanation: `${vocab.word} is pronounced as ${vocab.phonetic}`,
  });
...
async function seedDatabase() {
  ...
  try {
    ...
    for (let i = 0; i < totalWords; i++) {
      ...
      const topicIndex = categorizeWord(word);
      const level = determineLevel(i, totalWords);
      const translation = generateTranslation(word);

      const vocab = await prisma.vocabulary.create({
        data: {
          word: word,
          meaning: translation.meaning,
          meaningVi: translation.meaning,
          example: translation.example,
          exampleVi: translation.exampleVi,
          phonetic: translation.phonetic,
          topicId: createdTopics[topicIndex].id,
          level: level,
          order: i + 1,
        },
      });

      // Generate exercises for this vocabulary
      const exercises = generateExercises(vocab, WORDS);
      ...
      wordCount++;
      if (wordCount % 100 === 0) {
        console.log(`  ✓ Processed ${wordCount}/${totalWords} words`);
      }
    }
    ...
    console.log(`   - Vocabularies: ${wordCount}`);
    console.log(`   - Exercises: ${wordCount * 4} (4 per word)`);
  } catch (error) {
