...
async function main() {
  ...
  const presentationTopic = await prisma.topic.create({
    data: {
      ...
      description:
        "Vocabulary for delivering presentations and writing reports",
      icon: "📊",
      order: 5,
    },
  });
  ...
  await createWord({
    ...
    exampleVi:
      "Tôi muốn nhấn mạnh những thành tựu chính của chúng ta trong quý này.",
    phonetic: "/ˈhaɪlaɪt/",
    topicId: presentationTopic.id,
    order: 47,
    level: String(Level.INTERMEDIATE),
  ...
  await createWord({
    ...
    exampleVi:
      "Tóm lại, chúng ta đã vượt qua tất cả các mục tiêu trong quý này.",
    phonetic: "/ˈsʌməraɪz/",
    topicId: presentationTopic.id,
    order: 50,
    level: String(Level.INTERMEDIATE),
