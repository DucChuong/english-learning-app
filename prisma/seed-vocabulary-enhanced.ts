...
async function getWordDefinition(word: string) {
  try {
    ...
    return {
      ...
      meaning:
        data.meanings[0]?.definitions[0]?.definition || `Definition of ${word}`,
      example:
        data.meanings[0]?.definitions[0]?.example || `Example with ${word}`,
      audioUrl: data.phonetics?.find((p: any) => p.audio)?.audio || null,
    };
  } catch (error) {
    console.warn(`⚠️  Could not fetch definition for "${word}"`);
    return {
      phonetic: `/${word}/`,
...
// Simple Vietnamese translation (you can replace with Google Translate API)
async function getVietnameseTranslation(text: string): Promise<string> {
...
async function seedWithRealData() {
  console.log("🌱 Starting vocabulary seeding with real data...");
  ...
  try {
    ...
    const topicsData = [
      {
        ...
        nameVi: "Từ vựng cần thiết",
        icon: "⭐",
        order: 1,
      },
      {
        name: "Common Verbs",
        nameVi: "Động từ thông dụng",
        icon: "🏃",
        order: 2,
      },
      {
    ...
    console.log("\n📖 Processing vocabularies (this may take a while)...");
    ...
    for (let topicIndex = 0; topicIndex < topics.length; topicIndex++) {
      ...
      const endIndex = Math.min(startIndex + wordsPerTopic, WORDS.length);
      const topicWords = WORDS.slice(startIndex, endIndex);

      console.log(
        `\n  Processing topic: ${topic.name} (${topicWords.length} words)`
      );

      // Process words in smaller batches to avoid API rate limits
      const vocabularies = await batchProcess(
        topicWords,
        async (word) => {
          try {
            const definition = await getWordDefinition(word);
            const meaningVi = await getVietnameseTranslation(
              definition.meaning
            );
            const exampleVi = await getVietnameseTranslation(
              definition.example
            );

            // Determine level based on word frequency/position
            const globalIndex = startIndex + topicWords.indexOf(word);
            let level: Level;
            if (globalIndex < WORDS.length * 0.3) level = Level.BEGINNER;
            else if (globalIndex < WORDS.length * 0.7)
              level = Level.INTERMEDIATE;
            else level = Level.ADVANCED;
            ...
            const exercises = [
              {
                ...
                questionVi: `"${word}" có nghĩa là gì?`,
                options: [
                  meaningVi,
                  "Nghĩa khác 1",
                  "Nghĩa khác 2",
                ...
                explanation: `${word} means "${definition.meaning}"`,
              },
              {
                type: String(ExerciseType.FILL_BLANK),
                question: definition.example.replace(word, "______"),
                questionVi: exampleVi.replace(word, "______"),
                options: [],
                correctAnswer: word,
                explanation: null,
              },
            ...
            if (processedCount % 10 === 0) {
              console.log(
                `    ✓ ${processedCount}/${WORDS.length} words processed`
              );
            }
      ...
      console.log(`  ✓ Completed topic: ${topic.name}`);
    }

    console.log("\n✅ Seeding completed!");
    console.log(`📊 Final Summary:`);
    console.log(`   - Topics: ${topics.length}`);
    console.log(`   - Vocabularies: ${processedCount}`);
    console.log(`   - Exercises: ${processedCount * 2}`);
  } catch (error) {
